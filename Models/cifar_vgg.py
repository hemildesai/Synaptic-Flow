...
class VGG(nn.Module):
    def __init__(self, features, num_classes=10, init_weights=True, **kwargs):
        ...
        self.classifier = nn.Sequential(
            layers.TaylorLinear(512 * 7 * 7, 4096),
            nn.ReLU(True),
            nn.Dropout(),
            layers.TaylorLinear(4096, 4096),
            nn.ReLU(True),
            nn.Dropout(),
            layers.TaylorLinear(4096, num_classes),
        )
        if init_weights:
    ...
    def _initialize_weights(self):
        for m in self.modules():
            if isinstance(m, layers.TaylorConv2d):
                ...
                nn.init.constant_(m.bias, 0)
            elif isinstance(m, layers.TaylorLinear):
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == "M":
            ...
            conv2d = layers.TaylorConv2d(in_channels, v, kernel_size=3, padding=1)
            if batch_norm:
