    ...
    for name, module in model.named_modules():
        for pname, param in module.named_parameters(recurse=False):
            try:
                ...
                if pruned:
                    masked_sum = (
                        getattr(module, pname + "_mask").detach().cpu().numpy().sum()
                    )
                    total = getattr(module, pname + "_mask").detach().cpu().numpy().size

                    if hasattr(module, "skip_weight"):
                        # total += (
                        #     getattr(module, "skip_weight").detach().cpu().numpy().size
                        # )
                        masked_sum += np.count_nonzero(
                            getattr(module, "skip_weight").detach().cpu().numpy()
                        )

                    if hasattr(module, "skip_bias"):
                        # total += (
                        #     getattr(module, "skip_weight_2").detach().cpu().numpy().size
                        # )
                        masked_sum += np.count_nonzero(
                            getattr(module, "skip_bias").detach().cpu().numpy()
                        )
